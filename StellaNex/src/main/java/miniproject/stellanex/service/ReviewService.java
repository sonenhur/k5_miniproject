...
public class ReviewService {
	...
	public void save(String email, Long movie_id, int grade, String content, Long reviewId) {
		...
		Review review = Review.builder().movie(movie).grade(grade).content(content).writer(member).build();

		reviewRepository.save(review);
	}

	public List<ReviewInfoResponse> getAllReviewsByMovieId(Long movieId) {
		List<Review> reviews = reviewRepository.findActiveByMovieId(movieId);
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");

		return reviews.stream()
				.map(review -> ReviewInfoResponse.builder().review_id(review.getReview_id()).grade(review.getGrade())
						.content(review.getContent()).writer(review.getWriter().getEmail())
						.date(review.getDate().format(formatter)).build())
				.collect(Collectors.toList());
	}

	public List<ReviewInfoResponse> getAllReviewsByMovieIdOrderByRatingAsc(Long movieId) {
		List<Review> reviews = reviewRepository.findActiveByMovieIdOrderByGradeAsc(movieId);
		return mapReviewsToResponse(reviews);
	}

	public List<ReviewInfoResponse> getAllReviewsByMovieIdOrderByRatingDesc(Long movieId) {
		List<Review> reviews = reviewRepository.findActiveByMovieIdOrderByGradeDesc(movieId);
		return mapReviewsToResponse(reviews);
	}

	public List<ReviewInfoResponse> getAllReviewsByMovieIdOrderByDateAsc(Long movieId) {
		List<Review> reviews = reviewRepository.findActiveByMovieIdOrderByDateAsc(movieId);
		return mapReviewsToResponse(reviews);
	}

	public List<ReviewInfoResponse> getAllReviewsByMovieIdOrderByDateDesc(Long movieId) {
		List<Review> reviews = reviewRepository.findActiveByMovieIdOrderByDateDesc(movieId);
		return mapReviewsToResponse(reviews);
	}
	...
	public void delete(String email, Long reviewId) {
		...
		reviewRepository.softDeleteByEmail(email);
	}
